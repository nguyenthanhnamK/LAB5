...
GLboolean glslCompileString(GLSL_PROGRAM* p, GLSL_SHADER type, const GLchar* source)
{
	...
	glGetShaderiv(shader, GL_COMPILE_STATUS, &result); if (result == GL_FALSE) {
		int logLength = 0;
		glGetShaderiv(shader, GL_INFO_LOG_LENGTH, &logLength); if (logLength) {
			char* log = (char*)malloc(logLength); int size = 0;
			glGetShaderInfoLog(shader, logLength, &size, log); printf("%s\n", log);
			free(log);
		}
	}
	else {
